...
interface Props {
    ...
    dispatchIsReadyTodos: ActionDispatch<[action: any]>;
}

// set initial todos from local storage <- web api || predefined static
export function usePredefinedTodos({ dispatch, dispatchIsReadyTodos }: Props) {
    useEffect(() => {
        ...
        promisifiedTodos.then(payload => {
            dispatch({
                type: 'set',
                payload
            });
            dispatchIsReadyTodos({});
        });
    }, [dispatch]);
}